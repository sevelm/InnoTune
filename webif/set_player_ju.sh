...
if [ $LMS == "1" ]; then
   ...
   IPLMS="-s $(ip route show | grep 'src' | grep 'eth0' | awk '{print $9}' | tail -n1)"
else
...
#	echo $((PORT_BASE+3))
	...
	SPre=$(cat /opt/innotune/settings/settings_player/dev"$i".txt | head -n13 | tail -n1)
		...
		if [[ $APli ]]; then         ###Aktivate Player left on USB device "$i" - AirPlay
              /usr/local/bin/shairport -w -p $(($PORT_BASE+2)) -a "$PLAYERli" --on-start "echo 1 > /opt/innotune/settings/status_shairplay/status_shairplayli"".txt" --on-stop "echo 0 > /opt/innotune/settings/status_shairplay/status_shairplayli"$i".txt" -o alsa -- -d airplayli"$i" > /dev/null 2>&1 & echo $!
		fi
		...
		if [[ $SP ]]; then           ###Aktivate Player on USB device "$i" - AirPlay
            /root/librespot --name $PLAYER --cache /tmp --bitrate 320 --backend alsa --device airplay$i > /dev/null 2>&1
                                fi
		...
		if [[ $SQMAC ]]; then       ###Aktivate Player on USB device "$i" - Squeezbox
              /usr/bin/squeezelite-armv6hf -c flac,pcm,mp3,ogg,aac -b 2048:4096 -a 60:16::0 -r 44100 -p 97 -o plug:squeeze"$i" -n "$PLAYER" -m $SQMAC -z  > /dev/null 2>&1 & echo $!
